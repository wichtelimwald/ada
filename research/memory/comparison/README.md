...
Hindsight also uses pg0's bundled PostgreSQL. The dedicated research image adds the small Linux runtime pieces that pg0 needs inside a slim container: `libgssapi-krb5-2` plus `procps` (pg0 uses the external `ps` command for Unix process-liveness checks), and a passwd-visible non-root user matching the host UID/GID. These dependencies are confined to the disposable candidate sandbox; they are not installed on the host or in Ada's Dev Container.
