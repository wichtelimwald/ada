...
The accepted [Memory architecture](docs/decisions/ADR-0008-memory-architecture.md) is file-native and Markdown-first: current human-editable files are authoritative, private/shared Memory must map to enforceable protection domains, and automatic learning preserves separate dimensions for what knowledge is (for example a preference/fact/routine), how it arose (explicit statement, observed fact, behavioral observation, hypothesis), and its lifecycle/maturity. Promotion into established Memory crosses an Ada-owned validation boundary. **Only `learning/` ages automatically**: observed facts, behavioral observations, hypotheses and non-durable extracts/summaries live there until promoted or compacted. Established `memory/` content—including explicit facts/preferences and durable episodes—never ages or disappears automatically. Facts already owned by calendar/contact/document systems stay source-owned by default. Original documents/raw artifacts live outside Memory in a configured per-user/audience source store or provider; existing stable originals are referenced, while directly received files are first placed in that source store before extraction. Forgetting Memory never changes or deletes an original. Derived RAG/search indexes are rebuildable caches rather than independent truth. Claim/source/lifecycle information stays inspectable without a second drifting claim store, and Memory never grants permission or substitutes for action truth.
