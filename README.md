...
The accepted [Memory architecture](docs/decisions/ADR-0008-memory-architecture.md) is file-native and Markdown-first: current human-editable files are authoritative, private/shared Memory must map to enforceable protection domains, and automatic learning must preserve the distinction between explicit statements, observations, hypotheses and established Memory. Derived RAG/search indexes are rebuildable caches rather than independent truth. Claim/source/lifecycle information must remain inspectable without a second drifting claim store, and Memory never grants permission or substitutes for action truth.
