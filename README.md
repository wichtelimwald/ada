...
The intended lifecycle is: **empty Memory -> seed once -> active personality lives in user-controlled Memory**. From then on, personality may gradually learn and evolve while remaining inspectable, editable, and reversible through the local Memory history, while staying separate from permissions/privacy/action truth.
...
The MVP baseline starts with current-file reads and the simplest sufficient local search. Protected vault access is mediated by a **host-side Memory Broker**: Ada requests only the domain(s) needed for the current authorized task instead of holding standing access to every household vault. If RAG/FTS/vector/graph retrieval is added, it remains an automatically rebuildable scoped cache: candidate hits are resolved back to their current authoritative owner (Memory Markdown or the source-owned provider via the broker), lifecycle/maturity/scope are checked, and unavailable-source cached values are only last-known/unverified context. Git-style per-protection-domain history is implemented for the development file adapter through the Dulwich-backed adapter selected by [ADR-0009](docs/decisions/ADR-0009-memory-history-adapter.md); Git identity is not authentication. ReMe, LangMem, Hindsight and similar frameworks are optional derived components that must earn their added runtime and maintenance cost. A [synthetic control](research/memory/control/README.md) demonstrates the baseline behavior and failure modes that informed MVP-20. The development-only file adapter now adds opaque immutable IDs, explicit create-vs-correct semantics with optimistic revision checks, idempotent forget tombstones, lifecycle-aware current reads, fd-based no-follow file access, durable create/replace publication, create-only personality bootstrap, and local Git-style history that captures direct edits without making history semantic Memory. The Memory Broker, enforceable household protection domains, encryption, automatic retention/staleness, richer contradiction/current-superseded retrieval, and derived retrieval remain unimplemented, so this baseline is **not** approved for real household Memory.
...
Do not use that unprotected root for real household data. Local Git-style recovery history and safe-write semantics are implemented by MVP-20, but broker-mediated protection domains and encryption remain separate MVP gates.
