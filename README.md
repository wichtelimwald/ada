...
The accepted [Memory architecture](docs/decisions/ADR-0008-memory-architecture.md) is file-native and Markdown-first: current human-editable files are authoritative, private/shared Memory must map to enforceable protection domains, and automatic learning preserves separate dimensions for what knowledge is (for example a preference/fact/routine), how it arose (explicit statement, observed fact, behavioral observation, hypothesis), and its lifecycle/maturity. Promotion into established Memory crosses an Ada-owned validation boundary. Learning evidence is normally temporary and compacted rather than becoming a permanent behavioral archive; selected special conversations, workshops or events can instead be explicitly retained as durable episodic Memory. Facts that already belong to another authoritative system such as calendar/contact data stay source-owned by default; Memory references them or keeps genuinely derived knowledge instead of creating a stale second truth. Derived RAG/search indexes are rebuildable caches rather than independent truth. Claim/source/lifecycle information stays inspectable without a second drifting claim store, and Memory never grants permission or substitutes for action truth.
