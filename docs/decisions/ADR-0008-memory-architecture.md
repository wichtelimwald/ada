...
The combined executable gate under `research/memory/reme-langmem/` **passed on the target Mac** (2026-09-23, head `be34013`). The pinned dependencies installed together with Ada; the stdio tool allowlist, synthetic correction and conflict handling, silent-overwrite rejection, and retrieval after an out-of-band file edit passed. `pip-audit` reported zero known vulnerabilities for the dependencies it listed. The research [fit review](../../research/memory/reme-langmem/findings.md) records the raw-run limitations: this is a single synthetic vault, the Friday edit leaves prior source references without new Friday provenance, and metadata-level license triage is not a redistributed-wheel audit. This is evidence for the candidate, not adoption or a general proof of semantic integrity.
