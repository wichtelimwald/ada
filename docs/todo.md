...
- [ ] Implement the source/document store boundary with **independently configurable roots/providers per protection domain** (e.g. one user's iCloud, a separate family-share, or an external/encrypted volume); no central Ada `sources/` root is required. Reference existing durable originals in place, otherwise persist directly received files to that domain's configured root first using a human-browsable year/month organization; Memory never modifies/deletes originals, and only `learning/` extracts/summaries may age.
