...
- [ ] Implement retention semantics: only observation-derived evidence/hypotheses and non-durable derived summaries age/compact automatically; explicitly stated/confirmed facts and preferences never age or disappear automatically; durable episodes survive ordinary aging/compaction; all established Memory remains explicitly correctable/forgettable/deletable.
...
- [ ] Define the source/document store boundary for files handed to Ada: originals/raw artifacts are placed/referenced there and are never modified/deleted by Memory lifecycle; only Memory extracts/summaries/derived knowledge participate in aging/compaction.
